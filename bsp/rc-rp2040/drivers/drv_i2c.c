...
static rt_err_t dma_deinit(struct rp2040_i2c_device_info* i2c_dev) {
}

static void i2c_dma_irq_handler(struct rp2040_i2c_device_info* config) {
  // TODO: notify xfer routine that it can proceed with the next transfer
  rt_sem_release(&config->dma_completed);
}


static uint16_t* fill_dma_cmd_buffer(uint16_t* dma_cmd_buffer, struct rt_i2c_msg* msg) {
  ...
  if (msg->flags & RT_I2C_RD) {
    ...
    *dma_cmd_buffer |= I2C_IC_DATA_CMD_RESTART_BITS;
  }
}
                                    ...
                                    rt_uint32_t num)
{
  ...
  for (int i = 0; i < num; ++i) {
    ...
    uint16_t* dma_cmd_ptr = dev_info->dma_cmd_buffer;
    // TODO: make sure data to be sent does not exceed the internal buffer
    struct rt_i2c_msg* current_msg = &msgs[i];
    dma_cmd_ptr = fill_dma_cmd_buffer(dma_cmd_ptr, current_msg);
    *dma_cmd_ptr |= I2C_IC_DATA_CMD_STOP_BITS;
    ...
    dma_start_channel_mask(channel_trigger_mask);
    // TODO: wait until completed (with timeout)
    rt_sem_take(&dev_info->dma_completed, 0);
