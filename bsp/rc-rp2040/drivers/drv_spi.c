...
static rt_uint32_t rp2040_spi_xfer(struct rt_spi_device *device, struct rt_spi_message *message) {
  ...
  if (message->send_buf != RT_NULL && message->recv_buf != RT_NULL) {
    ...
    spi_write_blocking(hw->internal_bus_device, message->send_buf, message->length);
  } else {
...
static rt_uint32_t rp2040_spi_ext_xfer(struct rt_spi_device *device, struct rt_spi_message *message) {
  struct rp2040_spi_hw *hw = (struct rp2040_spi_hw *)device->bus->parent.user_data;
  struct rp2040_spi_ext_hw *ext_hw = rt_container_of(hw, struct rp2040_spi_ext_hw, hw);
  // wait until mbox is empty
  dma_channel_configure(ext_hw->tx_dma_channel, &ext_hw->tx_dma_config, &spi_get_hw(hw->internal_bus_device)->dr,
                        message->send_buf, message->length, false);

  dma_channel_configure(ext_hw->rx_dma_channel, &ext_hw->rx_dma_config, message->recv_buf,
                        &spi_get_hw(hw->internal_bus_device)->dr, message->length, false);
...
static struct rt_spi_ops rp2040_spi_ext_ops = {.configure = rp2040_spi_configure, .xfer = rp2040_spi_ext_xfer};
...
rt_err_t rp2040_spi_ext_hw_init(struct rp2040_spi_ext_hw *ext_hw) {
  ...
  rt_pin_write(ext_hw->int_pin, 0);
  dma_init(ext_hw);
  rt_pin_attach_irq(9, PIN_IRQ_MODE_FALLING, transaction_started, ext_hw);
